...
| **Elderly PWA** | Vite PWA port 5173, splash/login/home, 4 cognitive games, Dexie offline, Workbox, i18n en/as, cultural reminiscence, companion on splash/home/games |
...
| **Content** | Assamese/English cultural JSON packs, Manipuri POC pack, offline precache |
...
**~14% of full PDF vision** (engagement product + voice + 8-state packs + clinical validation).

**~40% of Tier 1 MVP scaffold** (4 games + sync + dashboard + offline paths exist but are not polish-ready for elders).

Cross-check: `task-registry.yaml` header and `TEAM_STATUS.md`.

## What teammates must do now

The product must feel **gamified** (themed game environments, reference art, reminiscence-linked play), not a bare utility grid. See per-member files in `docs/03-members/` and `NEXT_PLAN.md`.
